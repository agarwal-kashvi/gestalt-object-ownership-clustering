...
class ClusteringMetrics:
    def __init__(self, object_assignment_filename):
        print("Processing file: ", object_assignment_filename)
        # self.metrics_df = pd.read_csv(object_assignment_filename)[['ground_truth_location','predicted_location']].copy()
        self.metrics_df = pd.read_csv(object_assignment_filename)[['ground_truth_location','predicted_location','latitude','longitude']].copy()
    ...
    def clustering_recall(self):
        ...
        true_locs_list = self.metrics_df['ground_truth_location'].dropna().unique()
        ...
        for vineyard in TP_dict.keys():
            ...
            denom = TP_dict[vineyard] + FN_dict[vineyard]
            if denom == 0:
                recall = 0
            else:
                recall = TP_dict[vineyard] / denom
    ...
    def clustering_precision(self):
        ...
        for vineyard in TP_dict.keys():
            if str(vineyard) == "nan":  # not assigned a cluster, skip
                continue 
            denom = TP_dict[vineyard] + FP_dict[vineyard]

            if denom == 0:
                precision = 0
            else:
                precision = TP_dict[vineyard] / denom
